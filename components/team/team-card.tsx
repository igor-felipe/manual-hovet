...
export function TeamCard({ member }: TeamCardProps) {
  return (
    <div className="w-full max-w-sm  border bg-card text-card-foreground shadow-sm transition-all hover:shadow-lg">
        ...
        <div className="mb-4">
          {member.githubUrl + ".png" ? (
            <img
              ...
              alt={member.name}
              className="h-24 w-24 rounded-full object-cover translate-y-2 border-4 border-white shadow-md"
            />
          ) : (
            <div className="flex h-24 w-24 items-center justify-center rounded-full bg-muted translate-y-8">
      ...
      <div className="px-6 pb-2 h-[64px] -mt-5">
        <p className="text-center text-sm leading-6 line-clamp-3">
          {member.bio || ""}
        </p>
      </div>
